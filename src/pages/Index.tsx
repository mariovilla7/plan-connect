...
function Navbar() {
  ...
  return (
    ...
    >
      ...
      {open && (
        <nav className="lg:hidden bg-white/95 backdrop-blur-lg px-4 py-4 flex flex-col gap-4 border-t border-border/30 shadow-xl">
          {navLinks.map(({ label, id }) => (
            <button
              key={id}
              onClick={() => {
              ...
              className="text-sm font-bold text-muted-foreground hover:text-foreground text-left px-4"
            >
              {label}
            </button>
          ))}
  ...
  return (
    <div className="max-w-[1280px] mx-auto py-12">
      ...
      <div className="relative">
        <div
          ref={scrollRef}
          className="flex gap-6 overflow-x-auto pb-8 scrollbar-hide snap-x snap-mandatory px-4 -mx-4"
        >
...
function VideoSection() {
  return (
    <div className="max-w-[1024px] mx-auto text-center py-12">
      ...
      <div className="relative rounded-[40px] overflow-hidden bg-black border border-white/10 shadow-2xl group cursor-pointer">
        <img
          src={storytellingImg}
          alt="Demo"
...
function PricingSection() {
  return (
    <div className="max-w-[1280px] mx-auto py-20 text-center">
      <h2 className="text-3xl sm:text-4xl font-bold font-heading mb-12">Crecimiento sin Límites</h2>
      <div className="flex justify-center px-4">
        <div className="w-full max-w-md bg-[hsl(238,78%,54%)] rounded-[40px] p-10 text-white relative overflow-hidden shadow-2xl">
          ...
          <p className="mt-6 text-xs opacity-60 font-bold uppercase">
...
const featureCards = [
  { title: "Precisión clínica", desc: "Calcula kcal y macros en segundos con rigor científico.", image: gifPrecision },
  {
    ...
    desc: "Planes que respetan los ingredientes y cultura del paciente.",
    image: gifSustituciones,
  },
  { title: "Recálculo Inteligente", desc: "Cambia platos sin descuadrar los objetivos del día.", image: gifListas },
  { title: "Listas de Compra", desc: "Generación automática basada en el menú diseñado.", image: gifEducacion },
  { title: "Mensajes Educativos", desc: "Pautas personalizadas integradas en el plan.", image: gifEntrega },
  { title: "Exportación Ágil", desc: "PDFs listos para compartir por WhatsApp en un clic.", image: gifPrecision },
];
